...
function renderCharts() {
    ...
    function updateChartCounter(counterId, ownCount) {
        const el = document.getElementById(counterId);
        if (!el) return;
        
        if (combinedMode) {
            el.textContent = combinedCount;
        } else if (counterId.startsWith("acciones")) {
            el.textContent = filteredData.acciones.length;
        } else if (counterId.startsWith("processos")) {
            el.textContent = filteredData.processos.length;
        }
    }
    ...
    updateChartCounter("accionesCursoCount", accData.length);

    // --- Acciones: Sexo ---
    if (!combinedMode && tab === "acciones") {
        renderGender("accionesSexoContainer", "acciones");
        updateChartCounter("accionesSexoCount", filteredData.acciones.length);
    }
    ...
    updateChartCounter("accionesEtniaCount", accData.length);
    ...
    updateChartCounter("accionesZonaCount", accData.length);
    ...
    updateChartCounter("accionesComunaCount", accData.length);
    ...
    updateChartCounter("processosCursoCount", procData.length);

    // --- Processos: Se Identifican Como ---
    if (!combinedMode && tab === "processos") {
        renderGender("processosIdentificaContainer", "processos");
        updateChartCounter("processosIdentificaCount", filteredData.processos.length);
    }
    ...
    updateChartCounter("processosPreferenciaCount", procData.length);
    ...
    updateChartCounter("processosEtniaCount", procData.length);
    ...
    updateChartCounter("processosEscolaridadCount", procData.length);
    ...
    updateChartCounter("processosDiscapacidadCount", procData.length);
    ...
    updateChartCounter("processosZonaCount", procData.length);
    ...
    updateChartCounter("processosComunaCount", procData.length);
    ...
    updateChartCounter("processosSaludCount", procData.length);
